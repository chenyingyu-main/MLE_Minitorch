...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing when |a| <= 10.
    ...
    # one minus sigmoid is the same as sigmoid of the negative: they are NOT equal (=).
    assert minitorch.operators.is_close((1 - sigmoid(a)), sigmoid(-a))
    assert_close((1 - sigmoid(a)), sigmoid(-a))

    assert sigmoid(0) == 0.5
    if -10 <= a <= 10 and -10 <= a + 1 <= 10:
...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    # when using floats, they may not be exactly ==
...
def test_other() -> None:
    ...
    """Not sure whether we should raise an error or return a special value (such as infinity or NaN)"""
    assert inv(0.0) == float("inf")
    assert math.isnan(minitorch.operators.log(-0.5))
    assert minitorch.operators.log(0.0) == float("-inf")
    assert minitorch.operators.exp(0.0) == 1.0

    assert log_back(2.0, 3.0) == 1.5  # normal case
    assert log_back(0.0, -1.5) == float("-inf")  # 0.0 case
    assert math.isnan(log_back(-1.0, 1.0))  # negative case

    assert inv_back(2.0, 1.0) == -0.25  # normal case
    assert inv_back(0.0, 1.0) == float("-inf")
    assert inv_back(0.0, -1.0) == float("inf")
