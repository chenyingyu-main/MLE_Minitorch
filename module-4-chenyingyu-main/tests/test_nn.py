...
def test_max(t: Tensor) -> None:
    ...
    # Test 1: max(dim=0) - compare batch 0 vs batch 1
    # Expected: out[0,0,0] = max(t[0,0,0]=1, t[1,0,0]=13) = 13
    out = minitorch.max(t, 0)
    assert_close(out[0, 0, 0], max(t[0, 0, 0], t[1, 0, 0]))

    # Test 2: max(dim=1) - compare all channels
    # Expected: out[0,0,0] = max(t[0,0,0]=1, t[0,1,0]=5, t[0,2,0]=9) = 9
    out = minitorch.max(t, 1)
    assert_close(out[0, 0, 0], max([t[0, i, 0] for i in range(3)]))

    # Test 3: max(dim=2) - compare all widths
    # Expected: out[0,0,0] = max(t[0,0,0]=1, t[0,0,1]=2, t[0,0,2]=3, t[0,0,3]=4) = 4
    out = minitorch.max(t, 2)
    assert_close(out[0, 0, 0], max([t[0, 0, j] for j in range(4)]))
