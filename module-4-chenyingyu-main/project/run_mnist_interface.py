...
def render_run_image_interface():
    ...
    if st_train_button.button("Train Model"):
        ...
        def log_fn(epoch, total_loss, correct, total, losses, model):
            ...
            if hasattr(model, 'mid') and model.mid is not None:
                fig = px.imshow(-1 * model.mid.to_numpy()[0], facet_col=0)
                st_epoch_image.plotly_chart(fig, key=f"mid_viz_{epoch}_{timestamp}")
            ...
            st_epoch_plot.plotly_chart(fig, key=f"loss_plot_{epoch}_{timestamp}")
            print(
