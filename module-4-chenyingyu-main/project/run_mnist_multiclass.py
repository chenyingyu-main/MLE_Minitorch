...
class Network(minitorch.Module):
    ...
    def __init__(self):
        ...
        # TODO: Implement for Task 4.5.
        # Layer 1: Conv2d (1 → 4 channels, 3×3 kernel)
        self.conv1 = Conv2d(1, 4, 3, 3)
        
        # Layer 2: Conv2d (4 → 8 channels, 3×3 kernel)
        self.conv2 = Conv2d(4, 8, 3, 3)
        
        # Layer 3: Linear (392 → 64)
        # 392 = 8 channels × 7 height × 7 width
        self.linear1 = Linear(392, 64)
        
        # Layer 4: Linear (64 → 10)
        self.linear2 = Linear(64, C)

        # Why 392: Original: 28 × 28 (HxW)
        # ↓ Conv (3×3, same padding)
        # 28 × 28  (size unchange,  padding)
        # ↓ Conv (3×3, same padding)
        # 28 × 28
        # ↓ Pool (4×4)
        # 7 × 7    (28 ÷ 4 = 7)
        # ↓ Flatten (8 channels × 7 × 7)
        # 392
    ...
    def forward(self, x):
        ...
        # Step 1: Apply a convolution with 4 output channels and a 3x3 kernel followed by a ReLU (save to self.mid)
        # Conv1 (1 → 4) + ReLU
        # [batch, 1, 28, 28] → [batch, 4, 28, 28]
        self.mid = self.conv1(x).relu()
                
        # Step 2: Apply a convolution with 8 output channels and a 3x3 kernel followed by a ReLU (save to self.out)
        # Conv2 (4 → 8) + ReLU
        # [batch, 4, 28, 28] → [batch, 8, 28, 28]
        self.out = self.conv2(self.mid).relu()
        
        # Step 3: Apply 2D pooling (either Avg or Max) with 4x4 kernel.
        # [batch, 8, 28, 28] → [batch, 8, 7, 7]
        x = minitorch.maxpool2d(self.out, (4, 4))

        
        # Step 4: Flatten
        # [batch, 8, 7, 7] → [batch, 392]
        x = x.view(batch_size, x.shape[1] * x.shape[2] * x.shape[3])
        
        # Step 5: Linear1 (392 → 64) + ReLU + Dropout
        # [batch, 392] → [batch, 64]
        x = self.linear1(x).relu()
        x = minitorch.nn.dropout(x, self.dropout_rate, ignore=(not self.training))
