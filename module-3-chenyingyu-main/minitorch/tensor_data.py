...
def index_to_position(index: Index, strides: Strides) -> int:
    ...
    # index = (2, 3)
    # strides = (4, 1)
    # position = 2 * 4 + 3 * 1 = 8 + 3 = 11

    position = 0
    for idx, stri in zip(index, strides):
        position += idx * stri
    return position
...
def to_index(ordinal: int, shape: Shape, out_index: OutIndex) -> None:
    ...
    # out_index[1] = 11 % 4 = 3     # 11 divided by 4 gives remainder 3
    # ordinal = 11 // 4 = 2         # quotient is 2

    # out_index[0] = 2 % 3 = 2      # 2 divided by 3 gives remainder 2
    # ordinal = 2 // 3 = 0          # quotient is 0 --> done

    # output; out_index = (2, 3) ✓
    cur_ordinal = ordinal + 0
    for i in range(len(shape) - 1, -1, -1):

        out_index[i] = int(cur_ordinal % shape[i])
...
def broadcast_index(
    ...
    for i in range(len(shape)):
        if shape[i] == 1:
            ...
            dims_diff = len(big_shape) - len(shape)
            out_index[i] = big_index[i + dims_diff]

    # given a "big tensor index", figure out which element the "small tensor" should use.
    # that is to say, apply broadcasting rules at the index level
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    for i in range(max(len(shape1), len(shape2))):
        if shape1[i] != shape2[i] and shape1[i] != 1 and shape2[i] != 1:
            raise IndexingError(f"Unable to broadcast shapes {shape1} and {shape2}")
...
class TensorData:
    ...
    def index(self, index: Union[int, UserIndex]) -> int:
        ...
        if isinstance(index, tuple):
            ...
            # deal with list or already an array
            aindex = array(index)
