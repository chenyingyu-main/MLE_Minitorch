...
def tensor_data(
    ...
    # Ensure shape is not None
    # Determine the actual shape to use
    if shape is None:
        actual_shape: UserShape = draw(shapes())  # type: ignore[call-arg]
    else:
        actual_shape: UserShape = shape

    size = int(minitorch.prod(actual_shape))
    data = draw(lists(numbers, min_size=size, max_size=size))
    permute: List[int] = draw(permutations(range(len(actual_shape))))
    permute_shape = tuple([actual_shape[i] for i in permute])
    ...
    assert ret.shape[0] == actual_shape[0]
    return ret
